...
*/
	...
	for (uint32_t i = 0; i < timesteps; i++)
	{
		for (uint32_t j = 0; j < n_sensors; j++)
		{
			...
			rho(Trace[i*n_sensors + j], input, gamma, ptspace);
		}
	}
	...
	for (uint32_t i = 0; i < timesteps; i++)
	{
		for (uint32_t j = 0; j < n_sensors; j++)
		{
			for (uint32_t k = 0; k < dim*dim; k++)
			{
				...
				mpf_set_d(input, tmp);
				rho(P[i*dim*dim*n_sensors + j*dim*dim + k], input, gamma, ptspace);
			}
		}
	}
		...
		for (uint32_t j = 0; j < n_sensors; j++)
		{
			for (uint32_t k = 0; k < dim; k++)
			{
				...
				tmp = Px_double_array[i*dim*n_sensors + j*dim + k] *
				       traces[i*n_sensors + j];
				mpf_set_d(input, tmp);
				rho(Px[i*dim*n_sensors + j*dim + k], input, gamma, ptspace);
			}
		}
	}
